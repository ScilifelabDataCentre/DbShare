...
def create(dbname):
    ...
    if utils.http_GET():
        ...
        return flask.redirect(flask.url_for(".rows", dbname=dbname, viewname=viewname))


@blueprint.route(
    "/<name:dbname>/<name:viewname>/edit", methods=["GET", "POST", "DELETE"]
)
@utils.login_required
...
def rows(dbname, viewname):
    ...
    try:
        ...
        utils.flash_error(error)
        return flask.redirect(flask.url_for(".schema", viewname=str(viewname)))
    return flask.render_template(
        "view/rows.html",
        db=db,
        schema=schema,
...
def schema(dbname, viewname):
    ...
    has_write_access = dbshare.db.has_write_access(db)
    return flask.render_template(
        "view/schema.html", db=db, schema=schema, has_write_access=has_write_access
    )


@blueprint.route("/<name:dbname>/<name:viewname>/clone", methods=["GET", "POST"])
@utils.login_required
...
def download_csv(dbname, viewname):
    ...
    response.headers.set(
        "Content-Disposition", "attachment", filename="%s.csv" % viewname
    )
    return response
